...
CREATE PROCEDURE [dbo].[usp_SeleccionarTituloFormularioConStock]
	@Todo as nvarchar(1),
	@Proveedor int,
...
SELECT Titulos.Cod AS Cod, Titulos.NombreTitulo AS Titulo, Autores.Nombre AS Autor, Proveedores.Cuenta AS nProv, 
		...
		(SELECT SUM(StkPr) FROM TitulosStock WHERE TituloId = Titulos.Id) as CntPr,
		(SELECT SUM(StkCn) FROM TitulosStock WHERE TituloId = Titulos.Id) as CntCn
FROM  Titulos
